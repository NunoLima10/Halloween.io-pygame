            ...
            for coluna_index,coluna in enumerate(linha):
                ...
                if coluna!='-1':
                    if coluna=='3':
                        ...
                        bloco=bloco=Bloco((x,y),tamanho_bloco,'20')
                        #bloco.image.fill('red')
                        self.bloco_final.add(bloco) 
                    elif coluna=='2':
                        inimigo=Inimigo((x,y),2)
                        self.inimigos.add(inimigo)
                    elif coluna=='0':
                        bloco=bloco=Bloco((x,y),tamanho_bloco,'20')
                        #bloco.image.fill('red')
                        self.barreiras.add(bloco)
    ...
    def desenhar_decoracao(self,decoracao):
        ...
        for linha_index,linha in enumerate(decoracao):
            for coluna_index,coluna in enumerate(linha):
                ...
                if coluna!='-1':
                    decoracao=Decoracao((x,y),coluna)
                    self.decoracao.add(decoracao)
    ...
    def desenhar_terreno(self,mapa):
         ...
         for linha_index,linha in enumerate(mapa):
            for coluna_index,coluna in enumerate(linha):
                ...
                if coluna!='-1':
                    bloco=Bloco((x,y),tamanho_bloco,coluna)
        ...
        for sprite in self.blocos.sprites():
            if sprite.rect.colliderect(jogador.rect):
                if jogador.direcao.y>0:
                    ...
                    #voltando velocidade ao norma se tocar o chao
